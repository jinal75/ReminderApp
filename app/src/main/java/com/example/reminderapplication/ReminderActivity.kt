...
import com.example.reminderapplication.MainActivity
...
class ReminderActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mSubmitbtn!!.setOnClickListener {
            val title = mTitledit!!.text.toString()
                .trim { it <= ' ' } //access the data from the input field
            val date = mDatebtn!!.text.toString()
                .trim { it <= ' ' } //access the date from the choose date button
            val time = mTimebtn!!.text.toString()
                .trim { it <= ' ' } //access the time from the choose time button
            if (title.isEmpty()) {
    ...
    private fun processinsert(title: String, date: String, time: String) {
        val result: String = dbManager(this).addreminder(title,
            date,
            time) //inserts the title,date,time into sql lite database
    ...
    fun FormatTime(
        hour: Int,
        minute: Int,
    ): String {                                                //this method converts the time into 12hr format and assigns am or pm
        var time: String
        time = ""
    ...
    private fun setAlarm(text: String, date: String, time: String) {
        val am =
            getSystemService(ALARM_SERVICE) as AlarmManager //assigning alarm manager object to set alarm
        val intent = Intent(applicationContext, AlarmBrodcast::class.java)
        intent.putExtra("event",
            text) //sending data to alarm class to create channel and notification
        ...
        val intentBack = Intent(applicationContext,
            MainActivity::class.java) //this intent will be called once the setting alarm is complete
